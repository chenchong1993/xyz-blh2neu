...
int main()
{
	...
	//xyz[0] = -2645665.652062, xyz[1] = 3576603.708181, xyz[2] = 4555341.238256;//龙江楼顶
	xyz[0] = -2197958.2180, xyz[1] = 5180835.9775, xyz[2] = 2991563.9414;//长沙GW5
    ecef2pos(xyz, pos);                                                     //xyz转blh
    printf("pos: %.9f, %.9f, %.9f \n", pos[0], pos[1], pos[2]);

    FILE *fpRead  = fopen("C:\\Users\\chenc\\Desktop\\不规则格网\\不同高程测试\\最终数据\\不规则1000.txt", "r");     //需要修改
    FILE *fpWrite = fopen("C:\\Users\\chenc\\Desktop\\不规则格网\\不同高程测试\\最终数据\\不规则1000NEU.txt", "w+");   //需要修改
...
#if 1
		// 获取原始数据,输出经纬度数据，转成XYZ
        strncpy(temp, StrLine + 25, 13);
        ecefXYZ[0] = atof(temp);
        strncpy(temp, StrLine + 40, 12);
        ecefXYZ[1] = atof(temp);
        strncpy(temp, StrLine + 56, 12);
        ecefXYZ[2] = atof(temp);
#endif

        //printf("ecef %15.9f %15.9f %15.9f \n", ecefXYZ[0], ecefXYZ[1], ecefXYZ[2]);
        r[0] = xyz[0] - ecefXYZ[0];
        r[1] = xyz[1] - ecefXYZ[1];
        r[2] = xyz[2] - ecefXYZ[2];
        // dxyz
